...
/**
 * Find the MetaMask provider even when another wallet hijacks window.ethereum
 * Checks: providers array, providerMap, detected array, and direct property
 */
function findMetaMaskProvider(): EthereumProvider | null {
  ...
  // Method 1: providers array (EIP-6963 style)
  ...
  // Method 2: providerMap (some wallets use this)
  if (ethereum.providerMap) {
    const mm = ethereum.providerMap.get("MetaMask");
    if (mm && typeof mm.request === "function") return mm;
  }

  // Method 3: detected array
  if (ethereum.detected?.length) {
    const mm = ethereum.detected.find(
      (p: any) => p.isMetaMask && typeof p.request === "function"
    );
    if (mm) return mm;
  }

  // Method 4: direct MetaMask on window.ethereum
  if (ethereum.isMetaMask && typeof ethereum.request === "function") {
    return ethereum;
  }

  // Method 5: fallback — use whatever provider exists if it has request
...
/**
 * Check if MetaMask (or any compatible wallet) is installed
 */
...
/**
 * Get the Ethereum provider (MetaMask preferred, falls back to any available)
 */
